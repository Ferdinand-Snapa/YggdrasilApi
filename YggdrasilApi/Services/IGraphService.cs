...
namespace YggdrasilApi.Services
{
    public interface IGraphService
    {
        ...
        Task<bool> DeleatNodeAsync(string nodeId);
        Task<bool> UpdateNodeAsync(string nodeId, UpdateNodeRequest request);
        Task<(bool Success, string? Error, Connection? Connection)> AddConnectionAsync(int graphId, Dtos.CreateConnectionRequest request);
        Task<bool> SetNodeValuesAsync(string nodeId, System.Collections.Generic.IDictionary<string, object?> values);
    }
}
